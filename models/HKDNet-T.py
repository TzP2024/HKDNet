...
class CDAM(nn.Module):
    def __init__(self, in_channels, input_resolution, num_heads, window_size, depths, Group):
        ...
        # self.convr1 = nn.Sequential(nn.Conv2d(in_channels * 2, in_channels, 3, 1, 1), nn.BatchNorm2d(in_channels), nn.ReLU())
        # self.convd1 = nn.Sequential(nn.Conv2d(in_channels * 2, in_channels, 3, 1, 1), nn.BatchNorm2d(in_channels), nn.ReLU())
...
class RGBD_sal(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_RGB = shunted_b()
        self.conv_depth = shunted_b()
